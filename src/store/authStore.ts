...
/**
 * Migrate guest project to server after login
...
function loadProjectIntoIde(migratedProject: NonNullable<Awaited<ReturnType<typeof migrateGuestProject>>>) {
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  startOAuthLogin: () => {
    ...
    authClient.startOAuthLogin();
  },

  completeOAuthLogin: async (token: string) => {
    set({ isLoading: true, error: null });
    try {
      ...
      if (migratedProject) {
        loadProjectIntoIde(migratedProject);
      }
    } catch (error: any) {
      authClient.logout();
      set({
